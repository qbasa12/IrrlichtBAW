...
//				data[i+2] = output[j+0];
				...
				//os::Printer::log(std::to_wstring(data[i + 0]), ELL_DEBUG);
				data[i + 1] = (char)(output[j + 1] * (output[j + 3] / 255.f)); const char d = data[i + 1];
				//os::Printer::log(std::to_wstring(data[i + 1]), ELL_DEBUG);

				data[i+2] = (char)(output[j+0]*(output[j+3]/255.f));
				os::Printer::log(std::to_wstring(data[i + 2]), ELL_DEBUG);
			}
		}
		_IRR_ALIGNED_FREE(output);
	}
	else
		image = new asset::CImageData(output,nullOffset,imageSize,0,asset::EF_R8G8B8_SRGB,1,true);
